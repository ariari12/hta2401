...
window.onload = function() {
    ...
    // 게임이 중지된 상태에서 키보드의 아무 키나 누르면 게임 재시작
...
function drawScreen() {
    ...
    for (let i = 0; i < missileArray.length; i++) {
        ...
        // ctx.drawImage(missile, m.x - 27.5, m.y, 5, 20); // 왼쪽만 확인
        // ctx.drawImage(missile, m.x + 23.5, m.y, 5, 20); // 오른쪽만 확인
...
function checkCollision() {
    ...
    for (let i = 0; i < enemyArray.length; i++) {
        ...
        for (let j = 0; j < missileArray.length; j++) {
            ...
            let mx = j.idx % 2 == 0 ? m.x - 50 : m.x + 10;
            // let mx = m.x - 50; // 왼쪽 오차 수정
            // let mx = m.x + 10; // 오른쪽 오차 수정(확인중)
